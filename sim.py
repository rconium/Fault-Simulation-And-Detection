...
# -------------------------------------------------------------------------------------------------------------------- #
# FUNCTION: calculates the output value for each logic gate with a mode to determine wether to use faulty input/outputwires or not
def gateCalc(circuit, node, lineSpliced, mode=1):
...
def basic_sim(circuit, lineSpliced, mode):
    ...
    while True:
        ...
        if term_has_value:
            circuit[curr][2] = True
            circuit = gateCalc(circuit, curr, lineSpliced, mode)
...
def fault_sim(bench, f_input, f_list, jedi, sith):
    ...
    outputFailed.write("\ntotal detected faults: " + str(detected_fails) + "\n\n")
